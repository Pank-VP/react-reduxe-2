...
const MILLISECONDS_IN_YEAR =1000*60*60*24*365; // 1 year

const generateFaceDataItems = (index: number) => {
  return {
    id: `id-${index}`,
    ...
    createdAt: Date.now() - (random(1, 20, true) * MILLISECONDS_IN_YEAR),
...
const FAKE_DATA: IPlace[] = Array
  .from({length: 1000 })
  .map((item, index) => generateFaceDataItems(index));
